...
Router.route('/submit', {
	name: 'submitPage',
	waitOn: function() {
		return Meteor.subscribe('listAllCategories');
	}
});
...
Router.route('/category/:category_name', {
	name: 'categoryPage',
	waitOn: function() {
		return Meteor.subscribe('retrieveCategoryPage', this.params.category_name);
	},
	data: function() {
		return Categories.findOne({category_name: this.params.category_name});
	}
})
...
Router.route('/newTerm/:name', {
	name: 'newTerm',
	waitOn: function() {
		return Meteor.subscribe('retrieveNewTerm', this.params.name);
	},
	data: function() {
		return Dictionaries.findOne({name: this.params.name})
	}
});
...
Router.route('/submitSummary/:submitSummaryID' , {
	name: 'submitSummary',
	waitOn: function() {
		return Meteor.subscribe('lookupPost', this.params.submitSummaryID);
	},
	data: function() { 
